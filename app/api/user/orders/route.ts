...
/**
 * Get user orders from Polymarket
 * Per Polymarket docs: Orders are stored on-chain and can be queried via CLOB API
 * 
 * Query params:
...
export async function GET(request: NextRequest) {
  try {
    ...
    // NOTE: To get orders, we need API credentials which require a wallet signer
    // In server-side API route, we can't use walletClient directly
    // For now, return empty array - client-side should use the hook instead
    // TODO: In future, store API credentials in database or use server-side signing
    
    // Placeholder: Return empty orders for now
    // Client-side hooks (useUserOrders) should be used for actual order fetching
    return NextResponse.json({
      orders: [],
      userId: userId || null,
      eoaAddress: userAddress,
      proxyWalletAddress: proxyWalletAddress || null,
      message: 'Use client-side hooks (useUserOrders) for order fetching. Server-side requires wallet signer.',
    });
  } catch (error: any) {
